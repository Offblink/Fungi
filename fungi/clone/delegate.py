...
class DelegateTools:
    ...
    def send_file(self, args: dict) -> str:
        ...
        name = str(args.get("name") or "").strip() or Path(path).name
        ...
        staged = self.transport.upload_transfer(path, name, host)
        if staged.get("error"):
...
_SCHEMA_SEND_FILE = {
    ...
    "function": {
        ...
        "description": (
            ...
            "answer. Use this whenever the user asks to give/send a file to a "
            "friend host — never ask the peer's clone how to transfer files."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "host": {"type": "string", "description": "destination host name"},
                "path": {"type": "string", "description": "local file path on this machine"},
                "name": {
                    "type": "string",
