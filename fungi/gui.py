...
HELP_SECTIONS = [
    ...
    ("skill 与日记（实验性）",
     ...
     "接口——界面不展示、聊天流不显示调用痕迹，我们极不推荐去翻看它，"
     "它被问到也守口如瓶。"),
    ("手机端",
    ...
    ("文件与邮件",
     "对面发来的文件经你同意后落在仓库根 inbox/<来源主机>/；文字留言用 amail："
     "hub 收进对方邮箱（data/mail/），WebUI 顶栏 ✉ Amail 入口红点提示，"
     "点开模态框阅读、标记已读——两种投递都不惊动对方 Agent，离线也能收到。"),
    ("信使",
     "设置页开关（默认开）。开启时对面通讯 clone 的留言与文件请求由本机信使转述或推卡；"
     "关闭后全部直达：留言进会话视图（署名主机名）、卡片由 hub 直推、邮件静候邮箱，"
     "本机 Agent 完全不消耗。每个信封即时重读开关，无需重启。"
     "好友视图也可写：在输入框直接留言/发文件给对面，不经本机信使；"
     "对面信使开会转述并署「来自 ○○ 的用户」，关则直达对面界面。"),
    ("更多文档",
