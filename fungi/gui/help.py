...
HELP_SECTIONS = [
    ...
    ("信使",
     ...
     "定好的写进日历，剩下拿不准的才来问你。"
     "它每次汇报下面都留着一个输入框：想纠正它哪句、让它改自己写错的日程，写在那里就行"
     "——那是给自家信使的话，对面看不到。"
     "它向你提问不会把自己卡住：卡片等你有空再答，它先接着替你招待对面。"
     "朋友发来的文件永远要你点头才落盘；文字留言则不打扰任何人，离线也能收到。"
     "不想让 Agent 参与的话，关掉信使，Fungi 就成了一款简易版局域网微信。"),
    ("有人找你",
     "朋友的消息还没读时，托盘图标会闪，并且会响铃——默认就开着，可以在设置页「来信提醒」里关掉，"
     "也能在七个铃声里挑一个。点开那位好友的会话即视为已读，铃声随之停；"
     "正忙时用托盘菜单的「停止铃声」静音这一条，图标照旧闪到下一条被读到。"),
    ("skill 与日记",
    ...
    ("手机也能用",
     ...
     "走到哪了。"),
    ("关了窗口房间还在",
     "关闭窗口不停房：Fungi 缩进托盘继续守着。想真正离开，用托盘菜单的「退出」。"
     "再次启动程序会直接唤起主界面。"),
    ("设置",
