...
- **文件空间**（server 主机 `data/`；fs 守卫为白名单三分区）：
  ...
  完整数据目录图见下方「数据目录口径」。
- **send_file 传输与 amail 文字邮件**：send_file 把文件字节流在 hub 暂存（store-and-forward），接收方用户 consent 后落到对方 `inbox/<来源主机>/`；amail 是它的文字孪生——`amail` 工具把一封文字信投进对方在 hub 上的邮箱（`data/mail/<host>/`，server 权威、离线容忍），收件同样不惊动对面 Agent：WebUI 顶栏的 ✉ Amail 入口未读红点提示，点开模态框阅读、标记已读。
- **信使**：设置页开关（默认开）。开启时本机通讯 clone 醒来转述对面留言、推文件/consent 卡片；关闭后全部直达——留言直接进会话视图（署名主机名）、consent 卡片由 hub 直推、邮件静静躺进邮箱，本机 Agent 零消耗，即时生效。
- **好友视图可写**：WebUI（桌面/手机）好友视图底部有输入框——人类可直接留言或 📁 发文件给对面（`POST /comm-send` → `RoomBase.comm_send_human`），**不经本机信使**；收端语义由**对面**的信使开关决定：开则对面信使转述时如实署「来自 ○○ 的用户」，关则直达对面的会话视图/consent 卡片（人类发的文件发送侧免确认，接收侧确认不变；consent 卡片会注明落盘位置 `inbox/<来源主机>/`，投递成功后 hub 暂存副本自动清除——`data/transfers/` 只是中转暂存区，不是收件箱。）
