...
- **好友视图可写**：WebUI（桌面/手机）好友视图底部有输入框——人类可直接留言或 📁 发文件给对面（`POST /comm-send` → `RoomBase.comm_send_human`），**不经本机信使**；收端语义由**对面**的信使开关决定：开则对面信使转述时如实署「来自 ○○ 的用户」，关则直达对面的会话视图/consent 卡片（人类发的文件发送侧免确认，接收侧确认不变）。
